...
export const BrandsPage: React.FC = () => {
  ...
  const { data: brandsData, isLoading, refetch } = useQuery<ApiResponse<BrandDto[]>>({
    queryKey: ['admin-brands-list', statusFilter],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast('success', editingBrand ? 'Marka başarıyla güncellendi.' : 'Yeni marka başarıyla oluşturuldu.');
      closeModal();
    },
    onError: (err: unknown) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-brands-dropdown'] });
      queryClient.invalidateQueries({ queryKey: ['admin-dashboard-brands'] });
      showToast('success', 'Marka başarıyla silindi.');
      setBrandToDelete(null);
    },
    onError: (err: unknown) => {
      const error = err as AxiosError<{ message?: string }>;
      showToast('error', error.response?.data?.message || 'Marka silinirken bir hata oluştu.');
      setBrandToDelete(null);
    }
  });
