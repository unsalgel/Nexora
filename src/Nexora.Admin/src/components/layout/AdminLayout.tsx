            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-200">
              <div className="bg-white rounded-2xl max-w-md w-full shadow-xl border border-slate-200/80 p-6 space-y-5 animate-in zoom-in-95 duration-200">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Hesabınıza erişim sağlanmadan önce <span className="font-semibold text-slate-900">{securityNotice.failedCount} kez</span> hatalı parola girilmiştir:
                </p>

                {/* Başarısız Deneme Geçmişi Tablosu */}
                {securityNotice.details && securityNotice.details.length > 0 && (
                  <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100 text-xs">
                    ...
                    {securityNotice.details.slice(0, 5).map((item, idx) => (
